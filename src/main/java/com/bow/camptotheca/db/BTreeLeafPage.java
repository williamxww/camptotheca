...
public class BTreeLeafPage extends BTreePage {
    ...
    public int getMaxTuples() {
        // 每个tuple占用空间为tuple大小*8 + 1bit表示该slot是否被占用
        int bitsPerTupleIncludingHeader = td.getSize() * 8 + 1;
        // 3个int型的指针: left sibling pointer, right sibling pointer, parent
        // pointer
        int extraBits = 3 * INDEX_SIZE * 8;
        int tuplesPerPage = (BufferPool.getPageSize() * 8 - extraBits) / bitsPerTupleIncludingHeader;
        return tuplesPerPage;
    }

    /**
     * 计算所有tuple需要多少字节的header来表示slot的占用情况
     */
