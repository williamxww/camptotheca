                ...
                if (lockTable.get(pid).getType() == LockType.SLock) {
                    if (reqLock == LockType.SLock) {
                        ...
                        if (transactionTable.containsKey(tid) && transactionTable.get(tid).contains(pid)
                            ...
                            // this is a combined case when lock on pid hold
                            // only by one trans (which is exactly tid)
...
public class BufferPool {
            ...
            for (PageId pid : lockList) {
                ...
                if (pg != null) {
                    if (commit) {
                        ...
                        // all dirty pages are flushed and not dirty page are
                        // still in cache
                        // discard
                        discardPage(pid);
                    }
    ...
    private synchronized void flushPage(PageId pid) throws IOException {
        ...
        if (pgBufferPool.containsKey(pid)) {
            ...
            if (dirtier != null) {
                /*
                 * append an update record to the log, with a before-image and
                 * after-image.
                 */
