 ...
 * constructors.
 * 
 * @see com.bow.camptotheca.BTreeLeafPage#BTreeLeafPage
 * @see com.bow.camptotheca.BTreeInternalPage#BTreeInternalPage
 * @see com.bow.camptotheca.BTreeHeaderPage#BTreeHeaderPage
 * @see com.bow.camptotheca.BTreeRootPtrPage#BTreeRootPtrPage
 * @author Becca Taft
 */
public class BTreeFile implements DbFile {
    ...
    protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page,
        ...
        if (oldRightSibId != null) {
            BTreeLeafPage oldRightSib = (BTreeLeafPage) getPage(tid, dirtypages, page.getRightSiblingId(),
        ...
        /*
         * 
         * final int numEntries = page.getNumTuples(); final int midKeyNum =
         * numEntries / 2; BTreeLeafPage rightSib = (BTreeLeafPage)
         * this.getEmptyPage(tid, dirtypages, BTreePageId.LEAF);
         * 
         * int copyCnt = numEntries - midKeyNum; Iterator<Tuple> it =
         * page.reverseIterator(); Tuple[] toMove = new Tuple[copyCnt]; Field
         * midKey = null;
         * 
         * while (copyCnt > 0 && it.hasNext()) { toMove[--copyCnt] = it.next();
         * } midKey = toMove[0].getField(keyField); // delete and insert for
         * (int i = toMove.length - 1; i >= 0; i--) {
         * page.deleteTuple(toMove[i]); rightSib.insertTuple(toMove[i]); }
         * 
         * // copy up midkey BTreeInternalPage parent =
         * getParentWithEmptySlots(tid, dirtypages, page.getParentId(), midKey);
         * parent.insertEntry(new BTreeEntry(midKey, page.getId(),
         * rightSib.getId())); // rightSib.setParentId(page.getParentId());
         * 
         * // updatea parent ptr and sib ptrs BTreePageId origRightSibId =
         ...
         * 
         * if (origRightSibId != null) { BTreeLeafPage origRightsib
         * =(BTreeLeafPage) getPage(tid, dirtypages, origRightSibId,
         * Permissions.READ_WRITE);
         * origRightsib.setLeftSiblingId(rightSib.getId());
         * dirtypages.put(origRightSibId, origRightsib); }
         * dirtypages.put(page.getId(), page); dirtypages.put(rightSib.getId(),
         * rightSib); dirtypages.put(parent.getId(), parent);
         * 
         * if (field.compare(Op.GREATER_THAN, midKey)) { return rightSib; } else
         * { return page; }
         */
    }
    ...
    protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        /*
         * final int numEntries = page.getNumEntries(); final int midKeyNum =
         * numEntries / 2; BTreeInternalPage rightSib = (BTreeInternalPage)
         * this.getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL); // push up
         * mid key int copyCnt = numEntries - midKeyNum - 1;
         * 
         * Iterator<BTreeEntry> it = page.reverseIterator(); BTreeEntry[]
         * entryToMove = new BTreeEntry[copyCnt];
         * 
         * while (copyCnt > 0 && it.hasNext()) { entryToMove[--copyCnt] =
         * it.next(); } BTreeEntry midKey; if (it.hasNext()) { midKey =
         * it.next(); } else { throw new DbException(
         * "splitInternalPage: middle key should not be empty"); } for (int i =
         * entryToMove.length - 1; i >= 0; i--) {
         * page.deleteKeyAndRightChild(entryToMove[i]);
         * rightSib.insertEntry(entryToMove[i]); }
         * page.deleteKeyAndRightChild(midKey);
         * 
         * // push up midkey BTreeInternalPage parent =
         * getParentWithEmptySlots(tid, dirtypages, page.getParentId(),
         * midKey.getKey()); parent.insertEntry(new BTreeEntry(midKey.getKey(),
         * page.getId(), rightSib.getId()));
         * 
         * page.setParentId(parent.getId());
         * rightSib.setParentId(parent.getId()); updateParentPointers(tid,
         * dirtypages, rightSib);
         * 
         * dirtypages.put(page.getId(), page); dirtypages.put(rightSib.getId(),
         * rightSib); dirtypages.put(parent.getId(), parent);
         * 
         * if (field.compare(Op.GREATER_THAN, midKey.getKey())) { return
         * rightSib; } else { return page; }
         * 
         */
    }
    ...
    protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling, BTreeInternalPage parent,
        ...
        // assert sibling.getNumTuples() - numTupleToMove >=
        // (sibling.getMaxTuples() / 2);
        // assert page.getNumTuples() + numTupleToMove >= (page.getMaxTuples() /
        // 2);
     ...
     * @param leftEntry - the entry in the parent pointing to the two internal
     *        pages
     * @see #updateParentPointers(TransactionId, HashMap, BTreeInternalPage)
     * 
     * @throws DbException
    ...
    protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
            BTreeInternalPage page, BTreeInternalPage leftSibling, BTreeInternalPage parent, BTreeEntry leftEntry)
            throws DbException, IOException, TransactionAbortedException {
        // some code goes here
        // YOUR CODE HERE:
        ...
        for (int i = entryToMove.length - 1; i >= 0; --i) {
            ...
            // entry from leftsib must be inserted into first place in right
            // page
     ...
     * @param parent - the parent of the two internal pages
     * @param rightEntry - the entry in the parent pointing to the two internal
     *        pages
     * @see #updateParentPointers(TransactionId, HashMap, BTreeInternalPage)
     * 
     * @throws DbException
    ...
    protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
            BTreeInternalPage page, BTreeInternalPage rightSibling, BTreeInternalPage parent, BTreeEntry rightEntry)
            throws DbException, IOException, TransactionAbortedException {
        // some code goes here
        // YOUR CODE HERE:
     ...
     * @throws TransactionAbortedException
     */
    private BTreeRootPtrPage getRootPtrPage(TransactionId tid, HashMap<PageId, Page> dirtypages)
            throws DbException, IOException, TransactionAbortedException {
        synchronized (this) {
            if (f.length() == 0) {
